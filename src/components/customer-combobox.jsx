...
export function CustomerCombobox({ value, onChange, options }) {
  ...
  // If isOpen is triggered by the arrow button, we might want to show all options?
  // Or just filter by what's in the input. 
  // Standard behavior: show filtered. If user wants all, they clear input.
  // BUT, if user clicks arrow, they usually expect to see options.
  // Let's toggle: if query matches value exactly (selection made), show all?
  // No, simplest is: filter if query is not empty.
